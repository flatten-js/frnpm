...
const rnpm = (config) => {
  ...
  const exception = (message) => `${Combos.table.time}の${message}。`
  if (select > time) throw exception('制限を超えています')
  if (seconds > time) throw exception('数値が小さすぎます')
  ...
  const adjust = (month) => {
    ...
    if (month >= 12) {
      ...
      if (minute > 59) throw exception('数値が大きすぎます')
